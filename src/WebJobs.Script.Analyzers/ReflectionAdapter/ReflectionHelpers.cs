...
namespace Microsoft.Azure.Functions.Analyzers
{
    ...
    {
        private static bool TryMapAssembly(IAssemblySymbol asm, out Assembly asmRef)
        {
            asmRef = null;
#if false
        ...
        public static Attribute MakeAttr(JobHostMetadataProvider tooling, SemanticModel semantics, AttributeSyntax attrSyntax)
        {
            ...
            if (attrSyntax.ArgumentList != null)
            {
                foreach (var arg in attrSyntax.ArgumentList.Arguments)
                {
                    ...
                    {
                        argName = arg.NameEquals.ToString();
        ...
        // https://stackoverflow/questions/27105909/get-fully-qualified-metadata-name-in-roslyn
        ...
        private static Type MakeFakeType(ISymbol p2)
        {
            ...
            if (p2.Kind == SymbolKind.NamedType)
            {
                ...
                if (type.IsGenericType)
                {
                    ...
                    if (TryMapAssembly(asm, out asmRef))
                    {
                        // Reflection type. Important for unification with the binders
                    ...
                    return new GenericFakeType(definition, typeArgs);
                }
        ...
        public static Type GetParameterType(SyntaxNodeAnalysisContext context, ParameterSyntax parameterSyntax)
        {
            ...
            try
            {
                Type parameterType = MakeFakeType(p2); // throws if can't convert.
        ...
        private static bool IsOutParameter(this ParameterSyntax syntax)
        {
            ...
            {
                if (mod.IsKind(CodeAnalysis.CSharp.SyntaxKind.OutKeyword))
                {
                    return true;
                }
            }
